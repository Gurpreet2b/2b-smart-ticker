...
@Component({
  selector: 'app-archived-alert',
  templateUrl: './archived-alert.component.html',
  styleUrls: ['./archived-alert.component.css'],
})
export class ArchivedAlertComponent implements OnInit {
  ...
  IsArchivedSurveyFilter(event: any) {
  ...
  GetActiveList(page: number) {
    ...
    this.http.get(`alerts_database/survey_alerts_archieve_list/?search=${search}&survey_type=${this.filterType}`, null, { params: params }).subscribe((res: any) => {
